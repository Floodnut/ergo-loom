...
func systemLines(ctx core.PacketBuildContext) []string {
	lines := []string{
		...
		"",
		"Ergo Loom features:",
		"- Projects organize sessions by local path with configurable behavior policies.",
		"- Sessions are conversation threads. Each session can switch providers without losing context.",
		"- Queue: messages sent while a response is in progress are queued and run in order.",
		"- Steering: a message sent mid-response that interrupts and redirects the current reply.",
		"- Parallel: a request that runs on a different provider simultaneously as a candidate answer.",
		"- Candidate Output: a parallel result the user can accept or discard.",
	}
	if ctx.RouteLabel != "" {
